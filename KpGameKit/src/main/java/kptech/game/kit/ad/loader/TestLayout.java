...
public class TestLayout extends RelativeLayout {
    ...
    public TestLayout(Context context, ZadFeedDataAdBean adBean) {
        super(context);
        this.adBean = adBean;
...
//        renderFeedAdView(this.adBean);
    ...
    private void initView() {
        inflate(getContext(), R.layout.ad_popup_feed, this);
        findFeedView();
    }
    ...
    private void renderFeedAdView(ZadFeedDataAdBean ad) {
        switch (ad.getImageMode()) {
            case ZADFeedConstant.IMAGE_MODE_LARGE_IMG:
                ...
                if (ad.getImageList().size() > 0) {
                    ...
                    zmtItemAdImageView1.setVisibility(View.VISIBLE);
                    try {
                        Picasso.with(getContext()).load(ad.getImageList().get(0)).into(zmtItemAdImageView1);
                    }catch (Exception e){}
                } else {
            ...
            case ZADFeedConstant.IMAGE_MODE_VERTICAL_IMG:
                ...
                if (ad.getImageList().size() > 0) {
                    try {
                        Picasso.with(getContext()).load(ad.getImageList().get(0)).into(zmtItemAdImageView2);
                    }catch (Exception e){}
                }
                setContent(ad, zmtItemAdTitleTextView1, zmtItemAdDesTextView1);
                break;
            case ZADFeedConstant.IMAGE_MODE_SMALL_IMG:
                ...
                if (ad.getImageList().size() > 0) {
                    try {
                        Picasso.with(getContext()).load(ad.getImageList().get(0)).into(zmtItemAdImageView3);
                    }catch (Exception e){}
                }
            ...
            case ZADFeedConstant.IMAGE_MODE_GROUP_IMG:
                ...
                if (ad.getImageList().size() > 2) {
                    try {
                        Picasso.with(getContext()).load(ad.getImageList().get(0)).into(zmtItemAdImageView41);
                        Picasso.with(getContext()).load(ad.getImageList().get(1)).into(zmtItemAdImageView42);
                        Picasso.with(getContext()).load(ad.getImageList().get(2)).into(zmtItemAdImageView43);
                    }catch (Exception e){}
                }
        ...
        if (!TextUtils.isEmpty(ad.getAdLogoUrl())) {
            ...
            try {
                Picasso.with(getContext()).load(ad.getAdLogoUrl()).into(zmtItemLogoImageView1);
            }catch (Exception e){}
        } else {
