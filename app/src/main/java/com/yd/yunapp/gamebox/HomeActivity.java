...
import com.yd.yunapp.gamebox.view.loadmore.LoadMoreListView;
...
public class HomeActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private LoadMoreListView mGameList;
    ...
    private void initView() {
        ...
        mGameList = (LoadMoreListView) findViewById(R.id.game_list);
        if (mGameInfos.size() == 0) {
            mLoadingContainar.setVisibility(View.VISIBLE);
        } else {
            mLoadingContainar.setVisibility(View.GONE);
        }
        mGameList.setOnLoadMoreListener(new LoadMoreListView.OnLoadMoreListener() {
            @Override
            public void loadMore() {
                loadMoreData();
            }
        });
        mGameAdapter = new GameAdapter(this, mGameInfos.values());
    ...
    public void handleMessage(Message msg) {
        switch (msg.what) {
            case MSG_REFRESH_LIST:
                mGameAdapter.refresh(mGameInfos.values());
                mTips.setVisibility(View.GONE);
                mLoadingContainar.setVisibility(View.GONE);
                mGameList.onLoadMoreComplete();
                break;
            case MSG_SHOW_ERROR:
            ...
            case MSG_SHOW_NODATA:
                mGameList.setHasMoreData(false);
                break;
            default:
                break;
        }
