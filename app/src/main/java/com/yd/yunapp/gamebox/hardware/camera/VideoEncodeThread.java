...
import com.yd.yunapp.gamebox.hardware.sampler.SensorDataCallback;
...
public class VideoEncodeThread extends Thread {
    ...
    private void prepare() {
        Logger.i(TAG, "VideoEncoderThread().prepare");
        mFrameData = new byte[this.mWidth * this.mHeight * 3 / 2];
        mBufferInfo = new MediaCodec.BufferInfo();
        mCodecInfo = selectCodec(MIME_TYPE);
        if (mCodecInfo == null) {
            Logger.e(TAG, "Unable to find an appropriate codec for " + MIME_TYPE);
            return;
        }
    ...
    public void startMediaCodec() {
        if (!isStart) {
            try {
                ...
                isStart = true;
                synchronized (lock) {
                    Logger.e(TAG, Thread.currentThread().getId() + " video -- setConfigureReady...");
                    lock.notifyAll();
                }
            } catch (Exception e) {
                e.printStackTrace();
...
//                stopMediaCodec();
                ...
                try {
                    ...
                    Logger.e(TAG, "解码视频(Video)数据 失败", e);
                    e.printStackTrace();
                }
            }
        }
        Logger.e(TAG, "Video 录制线程 退出...");
    }
    ...
    public void release() {
        ...
        frameBytes = null;
        synchronized (lock) {
            Logger.e(TAG, Thread.currentThread().getId() + " video -- setExitReady...");
            lock.notifyAll();
        }
    }
    ...
    private void encodeFrame(byte[] input) {
        ...
        if (inputBufferIndex >= 0) {
            ...
            Logger.e(TAG, "input buffer not available");
        }

        int outputBufferIndex = mMediaCodec.dequeueOutputBuffer(mBufferInfo, TIMEOUT_ENCODE);
        Logger.i(TAG, "outputBufferIndex-->" + outputBufferIndex);
        do {
            if (outputBufferIndex == MediaCodec.INFO_TRY_AGAIN_LATER) {
                // just continue
                Logger.d(TAG, "outputBufferIndex = INFO_TRY_AGAIN_LATER");
            } else if (outputBufferIndex == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
                outputBuffers = mMediaCodec.getOutputBuffers();
            } else if (outputBufferIndex == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                MediaFormat newFormat = mMediaCodec.getOutputFormat();
            } else if (outputBufferIndex < 0) {
                Logger.e(TAG, "outputBufferIndex < 0");
            } else {
...
//                }
                if (mBufferInfo.size != 0) {
                    ...
                    Logger.d(TAG, "encode size : " + mBufferInfo.size + " , pts : " + mBufferInfo.presentationTimeUs);
                }
        ...
        if (mSensorDataCallback != null) {
            if ((data[4] & 0x1F) == 0x07) {
                ...
                if (spsLen > 0 && ppsLen > 0) {
                    ...
                    // just continue
                    Logger.d(TAG, " handleVideoData null");
                }
