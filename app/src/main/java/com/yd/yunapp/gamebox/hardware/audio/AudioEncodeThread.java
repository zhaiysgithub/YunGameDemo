package com.yd.yunapp.gamebox.hardware.audio;
...
public class AudioEncodeThread extends Thread {
    ...
    public void releaseMediaCodec() {
        Logger.d(TAG, "releaseMediaCodec");
        mIsStart = false;
    ...
    public void run() {
        ...
        if (DEBUG) {
            Logger.d(TAG, "AudioEncodeThread start encode  " + mIsStart + "   " + mIsExit);
        }
        try {
            while (!mIsExit) {
                if (!mIsStart) {
        ...
        if (DEBUG) {
            Logger.d(TAG, "AudioEncodeThread end encode");
        }
    }
